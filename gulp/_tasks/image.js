...
function image(device) {
  return gulp.src('./src/img/' + device + '/*.png')
  .pipe(imagemin({
    progressive: true,
    svgoPlugins: [{removeViewBox: false}],
    use: [pngquant()]
  }))
  .pipe(gulp.dest('./server/public/img/' + device));
}


function sprite(device) {
  var spriteData = gulp.src('./src/img/' + device + '/sprites/*.png')
  .pipe(spritesmith({
    imgName: 'sprite_' + version + '.png',
    cssName: '_sprite.scss',
    imgPath: '/img/' + device + '/sprite_' + version + '.png',
    cssFormat: 'scss',
